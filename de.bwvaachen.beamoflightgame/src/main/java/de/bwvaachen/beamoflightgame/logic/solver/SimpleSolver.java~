...
public class SimpleSolver {
	...
	public IBeamsOfLightPuzzleBoard solve(IBeamsOfLightPuzzleBoard model) {
		/*
		 * Verkn�pfung Index und festgestellte Anzahl an Abh�ngigkeiten
		 * Key=Index Value=
		 */
		...
		// Heraussuchen der Zahlenfelder und anlegen des L�sungsmodells
		for (int row = 0; row < model.getHeight(); row++) {
			for (int col = 0; col < model.getWidth(); col++) {
				ITile field = model.getTileAt(row, col);
				if (field instanceof INumberTile) {
		...
		for (Integer index : numberfieldsMap.keySet()) {
			...
			for (int dCol = -1; col + dCol >= 0
					&& model.getTileAt(row, col + dCol) instanceof ILightTile
					&& -dCol <= max; dCol--) {
		...
		// Herausstreichen der eindeutigen Abh�nigkeiten
		boolean change=true;
		while(change){
			change=false;
		...
		for (Integer index : numberfieldsMap.keySet()){
			NumberFieldDependenciesContainer dependenciesContainer = numberfieldsMap.get(index);
			INumberTile numberField = (INumberTile) model.getTileByIndex(index);
		...
		// MArkieren aller Felder an denen sich was ge�ndert haben k�nnte
		// Bei Gleichheit aktueller Wertigkeit und soll Wertigkeit entferne alle Abh�ngikeiten
